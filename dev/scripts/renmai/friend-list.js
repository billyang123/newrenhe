	...
	var firendList = Widget.extend({
		...
		delFriend:function(e){
			...
			Renhe.confirm('<span>'+i18n.popop[15]+'</span>',function(){
				$.ajax({url:url,data:{friendId:$this.data('fid')},dataType:"json",type:"post",
					success:function(res){
						$this.closest("li.def-card").remove();
					}
				})
			})
		},
		...
		addItemsToGroup:function(e){
			...
			this.setToGroup($this,{i18n:i18n.renmei,groupList:this.groupList,ids:ids},function(res,data){
				var cho = self.get('batch-list').find('.choosed');
				var lnm = $('[data-bind-renmai="num:0:'+data.groupId+'"]');
				var ln = parseInt(lnm.text(),10);
				databinder.set('chooseNum',0);
				databinder.set('group-'+self.groupCid[1]+'-count',self.num-cho.length);
				databinder.set('num:'+self.gid,self.num-cho.length);
				$('[data-gid="'+self.gid+'"]').attr("data-num",self.num-cho.length);
				databinder.set('num:0:'+data.groupId,ln+cho.length);
				$('[data-gid="0:'+data.groupId+'"]').attr("data-num",ln+cho.length);
				cho.remove();
				self.chooseNum-=cho.length;
				self.num-= cho.length;
			})
		...
		removeItemsFromGroup:function(e){
			...
			Renhe.confirm('<span>'+i18n.popop[13]+'</span>',function(){
				$.ajax({url:url,data:{id:ids},dataType:"json",type:"post",
					success:function(res){						
						var cho = self.get('batch-list').find('.choosed');
						var lnm = $('[data-bind-renmai="num:0:0"]');
						var ln = parseInt(lnm.text(),10);
						databinder.set('chooseNum',0);
						databinder.set('group-'+self.groupCid[1]+'-count',self.num-cho.length);
						databinder.set('num:'+self.gid,self.num-cho.length);
						$('[data-gid="'+self.gid+'"]').attr("data-num",self.num-cho.length);
						databinder.set('num:0:0',ln+cho.length);
						$('[data-gid="0:0"]').attr("data-num",ln+cho.length);
						cho.remove();
						self.chooseNum-=cho.length;
		...
		barsTabShow:function(e){
			...
			this.listRender(1);
			this.navtabEle && this.navtabEle.removeClass('active');
		...
		editGroup:function(e){
			...
			Renhe.popop(template.editGroup({i18n:i18n.popop,gid:this.groupCid[1],gname:this.name}),{
				...
				self.postData($( ele ),{
		    		...
		    		self.name = $( ele ).find('[name="gname"]').val();
		    		$('[data-gid="0:'+self.groupCid[1]+'"]').attr("data-name",self.name);
		    		databinder.set('group-'+self.groupCid[1]+'-name',self.name);
		...
		delGroup:function(e){
			...
			Renhe.confirm('<span>'+i18n.popop[11]+'</span><br><span>'+i18n.popop[12]+'</span>',function(){
				$.ajax({url:url,data:{gid:self.groupCid[1]},dataType:"json",type:"post",
					success:function(res){
						$('[data-gid="'+self.groupCid.join(':')+'"]').parent('li').remove();
					}
				})
			})
		},
				...
				buttons: [
					{
					    ...
					    click: function() {
					    	self.postData($( this ),{
					    		...
					    		self.groupList.push(res.group)
					    		self.get(target).find('li.nogroup').before('<li><a href="javascript:void(0);" data-num="0" data-name="'+res.group.groupname+'" data-gid="0:'+res.group.id+'"><span data-bind-renmai="group-'+res.group.id+'-name">'+res.group.groupname+'</span>   0</a></li>')
					    	})
					    	$( this ).dialog( "close" );
					    }
					}
