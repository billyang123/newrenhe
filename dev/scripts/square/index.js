define(function(require, exports, module) {
	...
	renheWidget.prototype = {
		...
		CommitSend:function(e,text){
			...
			$(text).hide().prependTo(apnode).removeClass("more-notice").slideDown();
		...
		like:function(e,data){
			...
			numTarget.closest('.media-praise-info').find(".cmt-s").append('<a href="/viewprofile.html?sid='+_data.memberSId+'">'+_data.memberName+'</a>').closest('.media-praise-info').slideDown();
		},
		...
		faceOvery:function(els){
			var self = this;
			Renhe.faceOvery(els,function(addFacePicId,options){
				...
				AddFaceEle.closest("form").find('button[type="submit"]').removeAttr("disabled");
			},function(dg){
				self.DGS["face"] = dg;
			})
		},
